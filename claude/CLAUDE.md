...
These instructions are the global baseline for Claude Code across repositories.
Project `CLAUDE.md` or `AGENTS.md` files may add local workflow, paths, commands,
and stricter rules, but they must not silently weaken this baseline.

## Rule Loading

Read the matching rule file in `~/.claude/rules/` before acting in that area:
...
2. Matching global `~/.claude/rules/*.md`
...
- When asked for review or when code changes are requested, use `/codex:review`
  when available, then iterate on findings with the updated diff for up to 3
  rounds. If that command is unavailable, use Claude Code's closest available
  review workflow and state the limitation.

Keep this file short. Put detailed durable rules in `~/.claude/rules/*.md`.
